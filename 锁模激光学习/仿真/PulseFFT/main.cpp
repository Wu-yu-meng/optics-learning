    ...
    SpatialFrame& intensity)
{
    ...
    {
        const double position_um = i * kDxUm;
        const double field = pulse.amplitude(now, position_um);
        electric_field[i] = field;
        intensity[i] = field * field;
    }
}
...
int main()
{
    ...
    const std::vector<OpticalWave> all_zero_waves = createWaves(reference_time, all_zero_phases);
    ...
    const GaussianPluse gaussian_pulse(
        kCenterFrequencyThz,
        0.0,
        kGaussianPulseAmplitudeAu,
        kGaussianPulseIntensityFwhmFs,
        reference_time);
    ...
    while (true)
    {
        ...
        const std::vector<OpticalWave> changing_random_phase_waves = createWaves(reference_time, changing_random_phases);
        calculateSpatialFrame(changing_random_phase_waves, now, E_frame, I_frame);
        const double unlocked_intensity_at_x0 = I_frame[0];
        ImgViz::enqueue_image_copy("1 Unlocked: changing random phases, I(x)",renderIntensity(I_frame));
        ...
        ImgViz::enqueue_image_copy("2 Locked: all phases equal, E(x)",renderElectricField(E_frame));
        ImgViz::enqueue_image_copy("2 Locked: all phases equal, I(x)",renderIntensity(I_frame));
        ...
        ImgViz::enqueue_image_copy("3 Locked: linear phase ramp, I(x)",renderIntensity(I_frame));
        ...
        ImgViz::enqueue_image_copy("4 Fixed random phases, I(x)",renderIntensity(I_frame));
